...
/client/proc/view_var_Topic(href, href_list, hsrc)
	...
	//~CARN: for renaming mobs (updates their name, real_name, mind.name, their ID/PDA and datacore records).
	...
	else if(href_list["transformicon"])
		if(!check_rights(R_FUN|R_BUILDMODE))	return

		var/result = input(usr, "Choose the transformation to apply", "Transform Mob") as null|anything in list("Scale", "Translate", "Rotate")
		var/atom/a = locate(href_list["transformicon"])
		...
		href_list["datumrefresh"] = href_list["transformicon"]
		...
		var/mob/H = locate(href_list["fix_vue"])

		if(!istype(H) || !H.client)
			to_chat(usr, "This can only be done on mobs with clients")
			return

		SSvueui.close_user_uis(H)
		H.client.cache.Cut()
		var/datum/asset/assets = get_asset_datum(/datum/asset/simple/vueui)
		assets.send(H)

		to_chat(usr, "Resource files sent")
		to_chat(H, "Your VueUI Resource files have been refreshed")

		log_admin("[key_name(usr)] resent the VueUI resource files to [key_name(H)] ")
